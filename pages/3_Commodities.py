...
<style>
    ...
    .hero h1 { font-size: 44px; font-weight: 950; line-height: 1.05; margin: 0 0 10px 0; color: #FFFFFF; letter-spacing: -0.8px; }
    .hero p  { margin: 0; font-size: 15px; color: rgba(255,255,255,0.82); line-height: 1.6; max-width: 920px; }
    .tag {
...
def risk_label_str(vol: float) -> str:
    ...
    if tickers:
        ...
        for i, t in enumerate(tickers[:4]):
            ...
            if currency in {"USD", "EUR"} and q.get("price_inr") is not None and q.get("fx_rate") is not None:
                ...
                cols[i].metric(label=t, value=f"{q['price']:.2f} {currency}", delta=delta)
    else:
...
with right:
    ...
    if not YF_AVAILABLE:
        st.error("yfinance is not available. Run: pip install yfinance")
    elif not st.session_state["run_commodities"]:
        ...
        if not tickers:
            ...
            if df_primary.empty or len(df_primary) < 5:
                st.error("Not enough data for the selected commodity. Try a longer history window.")
                st.session_state["run_commodities"] = False
            else:
