...
def simple_strength_rating(meta: dict) -> str:
    ...
    pb = meta.get("pb")
    roe = meta.get("roe")
    dte = meta.get("debt_to_equity")
    pm = meta.get("profit_margins")
    rg = meta.get("revenue_growth")
    eg = meta.get("earnings_growth")

    if isinstance(roe, (int, float)) and roe >= 0.12:
        score += 2
    elif isinstance(roe, (int, float)) and roe >= 0.07:
        score += 1

    if isinstance(pm, (int, float)) and pm >= 0.12:
        score += 2
    elif isinstance(pm, (int, float)) and pm >= 0.06:
        score += 1

    if isinstance(dte, (int, float)) and dte <= 80:
        score += 2
    elif isinstance(dte, (int, float)) and dte <= 160:
        score += 1
...
def portfolio_metrics(tickers_: list, weights: np.ndarray, days_: int):
    ...
    if run_portfolio:
        ...
        if len(ptickers) < 2:
            ...
            with st.spinner("Analyzing portfolio..."):
                pm = portfolio_metrics(ptickers, pweights, days_=max(days, 90))
...
"""
            ...
            with t2:
                ...
                if not meta:
                    ...
                    st.caption("Fundamentals are fetched from Yahoo Finance and may be incomplete for some tickers.")
